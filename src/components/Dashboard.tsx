...
import { Database, Users, MapPin, Clock, Calendar, Shield, Network, AlertTriangle, Info, Download } from 'lucide-react';
...
const REMEDIATION_TIPS: Record<string, string> = {
    'PII Density': 'Use email aliases and limit sharing personal info online',
    'Location Leakage': 'Disable location history and audit app permissions',
    'Financial Exposure': 'Use a dedicated email for financial services',
    'Social Mapping': 'Limit publicly visible contact lists on social media',
};

type DashboardTab = 'overview' | 'entities' | 'threats';
...
export default function Dashboard() {
    ...
    // Risk dimensions with contributing data
    const locationEvents = events.filter(e => e.category === 'location');
    const uniqueLocations = new Set(locationEvents.map(e => e.title)).size;
    ...
    const riskDims = [
        {
            label: 'PII Density',
            value: riskScore.piiDensity,
            desc: 'Personal identifiers found',
            detail: `${personEntities.length} person entities, ${entities.filter(e => e.type === 'organization').length} organizations identified`,
        },
        {
            label: 'Location Leakage',
            value: riskScore.locationLeakage,
            desc: 'Location patterns exposed',
            detail: `${uniqueLocations} unique locations from ${locationEvents.length} data points`,
        },
        {
            label: 'Financial Exposure',
            value: riskScore.financialExposure,
            desc: 'Financial data visible',
            detail: `${financialEvents.length} financial events detected across your data`,
        },
        {
            label: 'Social Mapping',
            value: riskScore.socialMapping,
            desc: 'Contact network revealed',
            detail: `${personEntities.length} contacts mapped across ${dataSources} data sources`,
        },
    ];
    ...
    return (
        <div className="page">
            ...
            <div className="dashboard-tabs">
                ...
                <button
                    className={`dash-tab ${activeTab === 'entities' ? 'active' : ''}`}
                ...
                <button
                    className={`dash-tab ${activeTab === 'threats' ? 'active' : ''}`}
                ...
                <div className="tab-content fade-in">
                    <div className="dashboard-grid">
                        ...
                        <div style={{ gridColumn: '1 / -1' }}>
                            ...
                            <div className="risk-breakdown" style={{ animationDelay: '0.2s' }}>
                                {riskDims.map((dim) => (
                                    <div key={dim.label} className="card risk-bar-card">
                                        ...
                                        <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 6 }}>{dim.detail}</div>
                                        <div className="remediation-tip">
                                            <Info size={10} />
                                            <span>{REMEDIATION_TIPS[dim.label]}</span>
                                        </div>
                                    </div>
                                ))}
                            </div>
